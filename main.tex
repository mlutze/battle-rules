...
\todo[inline]{this actually needs to consider the minimum attack/health ratio
since more health leads to lower hit chance}

One may select the maximum hit chance that they would like to see in the game.
For example, one may decide that the weakest unit should have a minimal chance of hitting
(i.e. require a maximal die roll to hit).
Taking all units into account and a given $H$ value, $P$ value, and $S$ value,
an $A$ value is selected such that the weakest unit will need to roll $S$ to hit.
...
\subsubsection{Example: Commoners Must Roll 20}

We consider again the 100 unarmored commoners example
with $H = 100$, $P = 2$, and $S = 20$.
We want them to have a 5\% chance to hit
(i.e., roll over 19).
We solve:
\begin{alignat*}{2}
    A &= 
